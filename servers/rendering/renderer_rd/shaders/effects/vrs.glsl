...
void main() {
	...
	frag_color = color.rg;
#else
	// Density map for VRS according to VK_KHR_fragment_shading_rate.

	// 4x8, 8x4 and 8x8 are only available on some GPUs.
	float max_texel_factor = 3.0; // TODO make this configurable so we can limit to 2.0 (4x4)

	color.r = floor(color.r * max_texel_factor + 0.1);
	color.g = floor(color.g * max_texel_factor + 0.1);
